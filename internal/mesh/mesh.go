...
func (n *Node) Start() error {
	...
	log.Printf("[mesh] node %s listening on %s with %d bootstrap peers",
		truncate(n.cfg.NodeID, 8), n.cfg.ListenAddr, len(n.cfg.Bootstrap))
	return nil
}
	...
	for {
		...
		if peerKey != nil && len(msg.Signature) > 0 {
			if !verifyHeartbeat(&msg, peerKey) {
				log.Printf("[mesh] dropping heartbeat with bad signature from %s", truncate(peerID, 8))
				continue
			}
		}
		...
		case <-ticker.C:
			...
			for id, p := range n.peers {
				if err := p.enc.Encode(hb); err != nil {
					log.Printf("[mesh] send to %s: %v", truncate(id, 8), err)
				}
			}
			n.mu.RUnlock()
		case <-n.stopCh:
